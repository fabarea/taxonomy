...
/**
 * @class TYPO3.Taxonomy.Application.AbstractBootstrap
 ...
 */
TYPO3.Taxonomy.Application.AbstractBootstrap = Ext.extend(Ext.util.Observable, {
	...
	addToMenu: function(path, items) {
		TYPO3.Taxonomy.Application.MenuRegistry.addMenuItems(path, items);
	},
	...
	getMenuItem: function(itemName) {
		var result = {};
		Ext.each(TYPO3.Taxonomy.Application.MenuRegistry.items.mainMenu, function(menuItem) {
	...
	handleNavigationToken: function(regexp, callback, scope) {
		scope = scope || this;
		TYPO3.Taxonomy.Application.on('TYPO3.Taxonomy.Application.navigate', function(token) {
			var matches = token && token.match(regexp);
			if (matches) {

				// As a first step, hides every panel.
				Ext.iterate(TYPO3.Taxonomy.UserInterface.contentArea.items.items, function (element) {
